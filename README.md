...
- **CLI**: `dolly list`, `show`, `edit` (opens `$EDITOR`, validates on save), `delete`, `link` (writes the `.dolly` marker into an existing project; `fit --apply` does it too), `home`, and `completions` for zsh, bash, and fish, with pattern names completing live
