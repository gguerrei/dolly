...
| `languages` | languages.programming | a code file whose extension belongs to a language outside the list (an ambiguous extension passes if any reading is sanctioned; an extension with a non-code reading is not code) | none: `fit` plans a translate step with the AI layer on ([translation.md](translation.md), ADR-0004) |
| `releases` | releases facet | `changelog: keep-a-changelog` with no changelog file at the root; a named `tool` whose root fingerprint is absent | create `CHANGELOG.md` with the format's header (and the semver line when `versioning` says so); the tool's config is report-only, like hooks. Versioning lives in tags, and `commits` in messages, so neither is a path to check (ADR-0005) |
