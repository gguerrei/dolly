...
Defaults live in this one table (mirrored in `ai/providers.ts`) and `dolly ai use --model` overrides per machine. A stale default is not a silent failure: `connect` verifies with a live call, so a retired model id surfaces immediately, in the provider's own error text.
