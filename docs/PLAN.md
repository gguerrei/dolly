...
## After the evaluation: the six gaps, closed

The 2026-09-10 evaluation left six things for after the first release, each with a plan here; the maintainer chose not to wait, and all six landed on 2026-09-11, each with its tests and its design doc updated. The engine publishes as `@dollysheep/core` from a library build ([RELEASING.md](RELEASING.md)); Playwright walks every GUI view in CI (`apps/desktop/e2e`, [gui.md](design/gui.md)); the marker is edited in place from the terminal, the daemon and the check view's `.dolly` panel ([check.md](design/check.md)); a `{name}` group's core is budgeted too ([extract.md](design/extract.md), "Layout"); a root without a manifest takes its toolchain and ecosystem from its members' vote ([extract.md](design/extract.md), "Toolchain"); and a `source:` URL's bundle is kept under the dolly home for an hour and pruned by `dolly home --prune` ([check.md](design/check.md)). The log carries the day.
